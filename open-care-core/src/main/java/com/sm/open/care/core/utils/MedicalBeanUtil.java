 ...
 */
public class MedicalBeanUtil {

	private static final Logger logger = LoggerFactory.getLogger(MedicalBeanUtil.class);
	...
	public static <T, V> V convert(T t, Class<V> clazz) {
		...
		try {
			...
			LoggerUtil.info(logger, "【core-MedicalBeanUtil-convert】, {0}, t:{1}",
	...
	public static <T, V> List<V> convertList(List<T> tList, Class<V> clazz) {
		...
		for(int i = 0; i < tList.size(); i ++) {
			t = tList.get(i);
			vList.add(MedicalBeanUtil.convert(t, clazz));
		}
		return vList;
	}
}
